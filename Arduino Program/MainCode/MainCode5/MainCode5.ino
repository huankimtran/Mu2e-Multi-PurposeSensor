...
#include "MPU6050.h"
#include <SD.h>
#include <DHT.h>    //Modified dht library, in which the data field is
                    //is set as public
/*-----------------PC-SOFTWARE-----------------
 * USE WITH THE PC PROGRAM RESIDING IN FOLDER
 * PC Program/Maincode/MainCode1.cpp
 */
/*---------------USED-PIN-----------------
...
class SDCARD{
  public: 
    ...
    void writeSetsOfAccData(accel_data dta[BUFFER_SIZE],int length,uint16_t temp,uint16_t humid){
  ...
  if(digitalRead(OPERATING_MODE_PIN)==READING_MODE){
    ...
    if(!digitalRead(INT_PIN)){
      ...
      if((++acelSen.index)==BUFFER_SIZE){
        ...
        acelSen.index=0;
        //SAVE DATA TO CARD
        card.writeSetsOfAccData(acelSen.buffer,BUFFER_SIZE,tempSen.getTemperature(),tempSen.getHumidity());
//       card.writeSetsOfAccData(acelSen.buffer,BUFFER_SIZE,0,0);
      }
    }
  }
}
